...
class Weapon(Component):
    ...
    def fire(self):
        if self.isReady() and self.selected and (
                self.isAutomatic or engine.clock.lastFrameTime > self.lastTrigger):
...
class Gun(Weapon):
    ...
    def __init__(self, actor, damage, modelFile, id):
        ...
        # So that if fire() is called between serverUpdate and clientUpdate,
        # ammo is still depleted
    ...
    def fire(self):
        if self.ammo > 0 and not self.reloadActive and self.showTime == - \
                1:  # We can't be switching weapons
        ...
        if self.actor.active:
            ...
            if self.showTime != -1:
                blend = min(1.0, (engine.clock.time -
                                  self.showTime) / self.totalShowTime)
            ...
            # Reloading sound (actually no sound right now)
            if self.activeSound == 1:
    ...
    def _updatePosition(self, offset, angleOffset):
        ...
        self.setPosition(self.actor.getPosition() +
                         (pos * (self.actor.radius + 0.1)) + Vec3(0, 0, offset))
...
class ChainGun(Gun):
    def __init__(self, actor, id):
        ...
        self.light = engine.Light(color=Vec4(
            1.0, 0.7, 0.4, 1), attenuation=Vec3(0, 0, 0.003))
    ...
    def clientUpdate(self, aiWorld, entityGroup, iterator=None):
        ...
        if engine.clock.time - self.lastFire > 0.1:
            ...
            self.light.setAttenuation(
                (0, 0, 0.005 + math.pow((engine.clock.time - self.lastFire), 2) * 8))
...
class MeleeClaw(Weapon):
    ...
    def serverUpdate(self, aiWorld, entityGroup, packetUpdate):
        ...
        if self.firing and self.active:
            ...
            if enemy is not None:
                ...
                vector.normalize()
                vector2.normalize()
                if math.acos(vector.getX() * vector2.getX() + vector.getY() * vector2.getY(
                ) + vector.getZ() * vector2.getZ()) < math.pi / 5 and vector3.length() < 8:
                    p.add(net.Boolean(True))
                    vector3.normalize()
                ...
                p.add(net.Boolean(False))
            self.firing = False
        elif self.active and self.impaleStart != -1 and self.impaleTarget is not None and self.impaleTarget.active and (self.actor.getPosition() - self.impaleTarget.getPosition()).length() < self.actor.radius + self.impaleTarget.radius + 0.5:
            self.addCriticalPacket(p, packetUpdate)
            ...
            # the explosion will already push us away.
            if self.impaleTarget.health > self.damage:
                self.actor.addForce(engine.impulseToForce(
                    self.impulseVector * -800))
    ...
    def clientUpdate(self, aiWorld, entityGroup, iterator=None):
        ...
        if iterator is not None:
            ...
            if state == 1:  # We're impaling
                ...
                if enemy is not None and enemy.active:
                    ...
                    particles.add(particles.HitRegisterParticleGroup(
                        pos, enemy.getTeam().color, 2))
        ...
        if self.selected and self.active:
            self.setPosition(self.actor.getPosition())
            self.node.lookAt(Point3(self.actor.controller.targetPos))
            if self.impaleStart != -1 and engine.clock.time - \
                    self.impaleStart > self.node.getDuration("Impale") and engine.clock.time - self.impaleStart < 0.75:
    ...
    def clientUpdate(self, aiWorld, entityGroup, iterator=None):
        ...
        if iterator is not None:
            if net.Boolean.getFrom(iterator):
                ...
                self.grenadeLaunchSound.play(
                    entity=entityGroup.getEntity(self.grenadeId))

        grenade = entityGroup.getEntity(self.grenadeId)
        if grenade is not None and isinstance(grenade, entities.Grenade):
            grenade.setActor(self.actor)
...
class MolotovThrower(Weapon):
    ...
    def clientUpdate(self, aiWorld, entityGroup, iterator=None):
        ...
        if iterator is not None:
            if net.Boolean.getFrom(iterator):
                # We're firing, play the launch sound. Everything else is taken
                # care of by the Grenade being spawned.
            ...
            if net.Boolean.getFrom(iterator):  # We're firing
                ...
                if net.Boolean.getFrom(iterator):  # Bullet hit something
                    ...
                    if net.Boolean.getFrom(iterator):
                        ...
                        pin = net.Boolean.getFrom(iterator)
                        ...
                        if entity is not None:
                            if pin:
                                ...
                                if not entity.pinned:
                                    entity.pin(
                                        hitPos - (direction * entity.radius))
