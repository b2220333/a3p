...
class GameUI(DirectObject):
    ...
    def __init__(self):
        self.localTeam = None
        self.aspectRatio = float(base.win.getProperties(
        ).getXSize()) / float(base.win.getProperties().getYSize())

        self.crosshairs = [None]
        self.crosshairs.extend(
            [
                self.loadCrosshair(
                    filename,
                    fitToScreen,
                    ...
                    fitToScreen,
                    alpha) in [
                    ("images/wide-crosshair.png",
                     False,
                     0.5),
                    ("images/narrow-crosshair.png",
                     False,
                     0.5),
                    ("images/sniper-scope-zoomed.png",
                     True,
                     1)]])
        ...
        self.healthBar = StatusBar(
            range=100, pos=(
                engine.aspectRatio - 0.02, 0, self.verticalOffset - 0.04), hpr=(
                0, 0, -90), width=0.075, height=0.5)

        self.scoreChangeText = OnscreenText(pos=(0, 0.4), scale=0.5, fg=(
            1, 1, 1, 0), font=visitorFont, mayChange=True)
        self.scoreChangeTextAlpha = 0.0
        self.scoreChangeTextScale = 0.1
        self.lastTeamScore = 0

        self.moneyText = OnscreenText(
            pos=(
                engine.aspectRatio - 0.53,
                self.verticalOffset - 0.05),
            scale=0.075,
        ...
        # 101 so it's in front of all the MeshDrawer particles.
        self.ammoTextNode.setBin("fixed", 101)
    ...
    def setTeams(self, teams, team):
        ...
        if self.localTeam.dock is not None:
        ...
        for i in range(len(self.teams)):
            ...
            bar = ScoreBar(range=100, pos=(xOffset, 0, yOffset),
                           hpr=(0, 0, 90), width=0.04, height=0.6)
            bar.setColors(fg=(1, 1, 1, 1), bg=(
                color.getX(), color.getY(), color.getZ(), 0.5))
            if self.hidden:
                bar.hide()
    ...
    def hide(self):
        ...
        if self.crosshairs[self.currentCrosshair] is not None:
    ...
    def update(self, scoreLimit):
        if self.hidden or self.localTeam is None:
            return
        ...
        self.specialBar.setValue(
            max(0, engine.clock.time - self.localTeam.lastSpecialActivated))
        ...
        for i in range(len(self.teams)):
            ...
            if team != self.localTeam:
                if player is not None and player.active:
                    self.playerUsernames[i].node().setTextColor(
                        team.color.getX() + 0.25, team.color.getY() + 0.25, team.color.getZ() + 0.25, 1)
                    self.playerUsernames[i].node().setText(player.username)
                    self.playerUsernames[i].setPos(
                        player.getPosition() + Vec3(0, 0, player.radius + 0.5))
                    self.playerUsernames[i].setScale(
                        pow((camera.getPos() - self.playerUsernames[i].getPos()).length(), 0.5) * 0.15)
        ...
        for i in range(len(allyList)):
            ...
            if img["image"] != file:
                img["image"] = file
                img.setTransparency(TransparencyAttrib.MAlpha)
            self.healthBars[i].setPos(
                actor.getPosition() + Vec3(0, 0, actor.radius + 1))
            self.healthBars[i].setValue(actor.health, actor.maxHealth)
            self.healthBars[i].setColor(actor.getTeam().color)
            self.healthBars[i].setScale(
                (camera.getPos() - self.healthBars[i].getPos()).length() * 0.07)
        ...
        if self.localTeam.score > self.lastTeamScore:
            ...
            self.scoreChangeText.setText(
                sign + str(int(self.localTeam.score - self.lastTeamScore)))
        ...
        if self.scoreChangeTextAlpha > 0.0:
            ...
            self.scoreChangeText["shadow"] = (
                0, 0, 0, self.scoreChangeTextAlpha * 0.5)
        ...
        if player is not None and player.active:
            ...
            if img["image"] != file:
                ...
                img.setTransparency(TransparencyAttrib.MAlpha)
            self.healthBar.show()
            if player.controller.targetedEnemy is not None and player.controller.targetedEnemy.active:
                particles.EnemySelectorParticleGroup.draw(
                    player.controller.targetedEnemy.getPosition(),
                    player.controller.targetedEnemy.radius)
            ...
            if isinstance(weapon, components.Gun) and weapon.selected:
                ...
                self.ammoTextNode.setPos(render.getRelativePoint(
                    weapon.node, Vec3(0.5, 0, 0.6)))
                # Angle the text toward the camera
                self.ammoTextNode.setH(self.ammoTextNode.getH() - 15)
            ...
            if player.health < self.lastPlayerHealth:
                self.damageTransparency += (self.lastPlayerHealth -
                                            player.health) / (.2 * player.maxHealth)
...
class ChatLog(DirectObject):
    ...
    def __init__(
        ...
        for i in range(maxChats):
            text = OnscreenText(pos=(-engine.aspectRatio + 0.02,
                                     verticalOffset + 0.18 + (i * 0.05)),
                                scale=0.035,
        ...
        self.chatBox = DirectEntry(text="",
                                   entryFont=font,
                                   pos=(-engine.aspectRatio + 0.02,
                                        0,
                                        verticalOffset + 0.13),
                                   scale=.035,
                                   text_fg=Vec4(1,
                                   ...
                                   initialText="",
                                   numLines=1,
                                   focus=1 if self.alwaysFocused else 0,
                                   rolloverSound=None,
                                   clickSound=None)
    ...
    def displayChat(self, username, message):
        self.messages.insert(0, Message(
            username + ": " + message, engine.clock.time))
        self._updateChatLog()
...
class UnitSelectorScreen(DirectObject):
    ...
    def __init__(self, startCallback):
        ...
        self.container = DirectFrame(parent=aspect2d, frameColor=(
            0.0, 0.0, 0.0, 0.0), frameSize=(-1.15, 1.15, -0.85, 0.75), pos=(0, 0, 0), sortOrder=-1)
        self.container.setBin("fixed", 0)
        self.background = OnscreenImage(image="menu/ui_background.png", pos=(
            0, 0, -0.05), parent=self.container, scale=(1.15, 1, 0.8))
        self.background.setTransparency(TransparencyAttrib.MAlpha)
        self.background.setColor(1, 1, 1, 0.5)

        visitorFont = loader.loadFont("menu/visitor2.ttf")
        self.balanceText = OnscreenText(
            parent=self.container, pos=Vec3(
                1.05, -0.55, 0), text="$0", align=TextNode.ARight, scale=0.1, fg=(
                1, 1, 1, 1), shadow=(
                0, 0, 0, 0.5), font=visitorFont, mayChange=True)
        inventoryLabel = OnscreenText(
            parent=self.container, pos=Vec3(
                0.7, -0.15, 0), text="Inventory", align=TextNode.ACenter, scale=0.07, fg=(
                1, 1, 1, 1), shadow=(
                0, 0, 0, 0.5), font=visitorFont, mayChange=False)
        playerLabel = OnscreenText(
            parent=self.container, pos=Vec3(
                -0.8, -0.575, 0), text="Player", align=TextNode.ACenter, scale=0.07, fg=(
                1, 1, 1, 1), shadow=(
                0, 0, 0, 0.5), font=visitorFont, mayChange=False)
        unitQLabel = OnscreenText(
            parent=self.container, pos=Vec3(
                -0.15, -0.575, 0), text="Unit Q", align=TextNode.ACenter, scale=0.07, fg=(
                1, 1, 1, 1), shadow=(
                0, 0, 0, 0.5), font=visitorFont, mayChange=False)
        unitELabel = OnscreenText(
            parent=self.container, pos=Vec3(
                0.2, -0.575, 0), text="Unit E", align=TextNode.ACenter, scale=0.07, fg=(
                1, 1, 1, 1), shadow=(
                0, 0, 0, 0.5), font=visitorFont, mayChange=False)
        undoButton = DirectButton(
            parent=self.container, text="Undo", pos=(
                0.62, 0, -0.75), relief=DGG.FLAT, text_font=visitorFont, frameSize=(
                -0.4, 0.4, -.15, .15), frameColor=(
                0.0, 0.0, 0.0, 0.5), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.35, rolloverSound=None, clickSound=None, command=self.undo)
        startButton = DirectButton(
            parent=self.container, text="Start", pos=(
                0.96, 0, -0.75), relief=DGG.FLAT, text_font=visitorFont, frameSize=(
                -0.45, 0.45, -.15, .15), frameColor=(
                0.0, 0.0, 0.0, 0.5), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.35, rolloverSound=None, clickSound=None, command=self.startCallback)

        self.infoDialog = DirectFrame(sortOrder=100, frameColor=(
            0.03, 0.03, 0.03, 1.0), frameSize=(0, 1.05, -0.3, 0.3), pos=(0, 0, 0))
        self.infoDialog.hide()
        self.infoTitle = OnscreenText(
            pos=Vec3(
                0.075, 0.2, 0), parent=self.infoDialog, align=TextNode.ALeft, scale=0.1, fg=(
                1, 1, 1, 1), font=visitorFont, mayChange=True)
        self.infoCostText = OnscreenText(
            pos=Vec3(
                0.97, 0.2, 0), parent=self.infoDialog, align=TextNode.ARight, scale=0.1, fg=(
                1, 1, 1, 1), font=visitorFont, mayChange=True)
        dejavuFont = loader.loadFont("menu/DejaVuSans.ttf")
        self.infoText = OnscreenText(
            pos=Vec3(
                0.075, 0.12, 0), parent=self.infoDialog, wordwrap=18, align=TextNode.ALeft, scale=0.03, fg=(
                1, 1, 1, 1), font=dejavuFont, mayChange=True)
        self.infoPromptText = OnscreenText(
            pos=Vec3(
                0.97,
                -0.24,
                0),
            text="Click to purchase",
            parent=self.infoDialog,
            align=TextNode.ARight,
            scale=0.05,
            fg=(
                1,
        ...
        for i in range(6):
            ...
            slot = UnitIconSlot(
                code,
                UnitIconSlot.AcceptsWeapons,
                origin + offset,
                "images/buy-slot.png",
                UnitSelectorScreen.types[code][1],
                isSpecial=False)
        ...
        for x in range(4):
            for y in range(4):
                slot = UnitIconSlot(-1, UnitIconSlot.AcceptsAny,
                                    origin + Vec3(x * 0.2, 0, -y * 0.2))
                self.inventorySlots.append(slot)

        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsWeapons,
                            Vec3(-1.0, 0, -0.7), "images/weapon-slot.png")
        self.playerSlots.append(slot)
        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsWeapons,
                            Vec3(-0.8, 0, -0.7), "images/weapon-slot.png")
        self.playerSlots.append(slot)
        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsSpecials,
                            Vec3(-0.6, 0, -0.7), "images/special-slot.png", isSpecial=True)
        self.playerSlots.append(slot)

        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsWeapons,
                            Vec3(-0.35, 0, -0.7), "images/weapon-slot.png")
        self.unitSlots.append(slot)
        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsSpecials,
                            Vec3(-0.15, 0, -0.7), "images/special-slot.png", isSpecial=True)
        self.unitSlots.append(slot)

        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsWeapons,
                            Vec3(0.1, 0, -0.7), "images/weapon-slot.png")
        self.unitSlots.append(slot)
        slot = UnitIconSlot(-1, UnitIconSlot.AcceptsSpecials,
                            Vec3(0.3, 0, -0.7), "images/special-slot.png", isSpecial=True)
        self.unitSlots.append(slot)
        ...
        if not self.hidden:
            ...
            if not selected:
                for slot in self.buySlots:  # Check if the user is clicking a buy slot
                    if slot.icon is None and (
                            slot.getPos() - pos).length() < 0.1:
                        if self.team.purchaseItem(slot.type):
                            icon = UnitSelectIcon(
                                slot.type, slot.isSpecial, UnitSelectorScreen.types[slot.type][0])
    ...
    def release(self):
        if self.pressed and self.selectedIcon is not None:
            closestSlot = None
            closest = 10
            for slot in (
                x for x in self.buySlots +
                self.inventorySlots +
                self.playerSlots +
                self.unitSlots if x.icon is None and (
                    x.type == self.selectedIcon.type or x.type == -
                    1)):
            ...
            if closestSlot is not None and closest < 0.2:
        ...
        if not self.hidden:
            ...
            for slot in self.buySlots:
                if (slot.getPos() - pos).length() < 0.1:
                    ...
                    self.infoTitle.setText(
                        UnitSelectorScreen.types[slot.type][1])
                    ...
                    self.infoText.setText(
                        UnitSelectorScreen.types[slot.type][2])
                    showInfo = True
                    break
            if not showInfo:
                self.infoDialog.hide()
    ...
    def _getMousePos(self):
        ...
        return Vec3(((float(x) / width) - 0.5) * aspectRatio *
                    2, 0, (-float(y) / height) * 2.0 + 1.0)

    def show(self):
        self.hidden = False
        engine.Mouse.showCursor()
        for slot in self.buySlots + self.inventorySlots + \
                self.playerSlots + self.unitSlots:
            slot.show()
    ...
    def getUnitWeapons(self):
        return [
            None if x.icon is None else x.icon.type for x in self.unitSlots if not x.isSpecial]

    def getUnitSpecials(self):
        return [
            None if x.icon is None else x.icon.type for x in self.unitSlots if x.isSpecial]

    def getPrimaryWeapon(self):
        icon = self.playerSlots[0].icon
        return None if icon is None else icon.type
    ...
    def __init__(self, range, pos, hpr, width, height):
        ...
        self.usernameText = OnscreenText(
            ...
            fg=(
                ...
                1,
                1),
            font=loader.loadFont("menu/visitor2.ttf"),
            mayChange=True)
...
class UnitStatusBar(StatusBar3D):
    ...
    def __init__(self):
        ...
        # 101 so it's in front of all the MeshDrawer particles.
        self.textNode.setBin("fixed", 101)
...
class EditorUI(DirectObject):
    ...
    def __init__(self):
        ...
        self.crosshairs = OnscreenImage(
            image="images/narrow-crosshair.png", pos=(0, 0, 0), scale=0.1)
...
class Menu(DirectObject):
    ...
    def __init__(self):
        self.active = True
        visitorFont = loader.loadFont("menu/visitor2.ttf")
        self.dialog = DirectFrame(frameColor=(
            0.1, 0.4, 0.6, 0.6), frameSize=(-.45, .45, -.3375, .4375), pos=(0, 0, 0))
        self.postProcessingCheckBox = DirectCheckButton(
            parent=self.dialog,
            text="Post-processing",
            indicatorValue=engine.enablePostProcessing,
            pos=(
                0,
                0,
                0.325),
            ...
            text_scale=2.0,
            boxImage=(
                "images/checkbox-disabled.jpg",
                "images/checkbox-enabled.jpg",
                None),
            frameColor=(
                0,
                0,
            ...
            rolloverSound=None,
            clickSound=None,
            command=self.togglePostProcessing)
        self.shadersCheckBox = DirectCheckButton(
            parent=self.dialog,
            text="Shaders",
            indicatorValue=engine.enableShaders,
            pos=(
                0,
                0,
                0.2),
        ...
        self.distortionEffectsCheckBox = DirectCheckButton(
            parent=self.dialog,
            text="Distortion effects",
            indicatorValue=engine.enableDistortionEffects,
            pos=(
                0,
                0,
                0.1),
            ...
            rolloverSound=None,
            clickSound=None,
            command=self.toggleDistortionEffects)
        self.shadowsCheckBox = DirectCheckButton(
            parent=self.dialog,
            text="Shadows",
            indicatorValue=engine.enableShadows,
            pos=(
                0,
                0,
                -0.0),
            ...
            rolloverSound=None,
            clickSound=None,
            command=self.toggleShadows)
        self.antialiasingCheckBox = DirectCheckButton(
            parent=self.dialog,
            text="Antialiasing",
            indicatorValue=engine.enableAntialiasing,
            pos=(
                0,
                0,
                -0.1),
        ...
        exitButton = DirectButton(
            parent=self.dialog, text="Exit", pos=(
                -0.25, 0, -0.26), relief=DGG.FLAT, text_font=visitorFont, frameSize=(
                -0.5, 0.5, -.15, .15), frameColor=(
                0.0, 0.0, 0.0, 0.5), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.35, rolloverSound=None, clickSound=None, command=self.delete)
        resumeButton = DirectButton(
            parent=self.dialog, text="Resume", pos=(
                0.25, 0, -0.26), relief=DGG.FLAT, text_font=visitorFont, frameSize=(
                -0.5, 0.5, -.15, .15), frameColor=(
                0.0, 0.0, 0.0, 0.5), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.35, rolloverSound=None, clickSound=None, command=self.toggle)
        self.dialog.hide()
        self.hidden = True
...
class HostList(DirectObject):
    ...
    def __init__(self, callback):
        ...
        visitorFont = loader.loadFont("menu/visitor2.ttf")
        self.dialog = DirectFrame(frameColor=(
            0.0, 0.0, 0.0, 0.0), frameSize=(-.9, .9, -.9, .9), pos=(0, 0, 0))
        self.background = OnscreenImage(
            image="menu/ui_background.png",
            pos=(
                0,
                0,
                0),
        ...
        self.background.setColor(1, 1, 1, 0.5)
        self.label = OnscreenText(
            parent=self.dialog, text="Choose server", pos=(
                0, .825), font=visitorFont, fg=(
                1, 1, 1, 1), scale=0.1)
        self.serverIpEntry = DirectEntry(
            parent=self.dialog,
            pos=(
                -0.55,
                0,
            ...
            frameColor=(
                ...
                0,
                0.5),
            initialText="Direct Connect",
            numLines=1,
            rolloverSound=None,
            clickSound=None,
            focus=0,
            focusInCommand=self.clearServerIp,
            command=self.go)
        self.cancelButton = DirectButton(
            parent=self.dialog, text="Cancel", pos=(
                -0.65, 0, -0.8), relief=DGG.FLAT, text_font=visitorFont, frameColor=(
                0, 0, 0, 0.5), frameSize=(
                -0.5, 0.5, -.15, .15), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.3, rolloverSound=None, clickSound=None, command=self.hide)
        self.refreshButton = DirectButton(
            parent=self.dialog, text="Refresh", pos=(
                0.65, 0, -0.8), relief=DGG.FLAT, text_font=visitorFont, frameColor=(
                0, 0, 0, 0.5), frameSize=(
                -0.6, 0.6, -.15, .15), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.3, rolloverSound=None, clickSound=None, command=online.getHosts)
        self.joinButton = DirectButton(
            parent=self.dialog, text="Join", pos=(
                0.4, 0, -0.68), relief=DGG.FLAT, text_font=visitorFont, frameColor=(
                0, 0, 0, 0.5), frameSize=(
                -0.4, 0.4, -.14, .14), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.3, rolloverSound=None, clickSound=None, command=self.go)
        self.serverList = DirectScrolledFrame(
            ...
            verticalScroll_frameColor=(
                1,
                1,
                1,
                0.2),
            verticalScroll_pageSize=0.4,
            ...
            verticalScroll_thumb_image="images/checkbox-disabled.jpg",
            verticalScroll_thumb_frameColor=(
                0,
                0,
                0,
                0),
            verticalScroll_thumb_scale=0.04,
            verticalScroll_thumb_image_scale=0.04,
            verticalScroll_incButton_image="images/checkbox-disabled.jpg",
            verticalScroll_incButton_frameColor=(
                0,
                0,
                0,
                0),
            verticalScroll_incButton_scale=0.04,
            verticalScroll_incButton_image_scale=0.04,
            verticalScroll_decButton_image="images/checkbox-disabled.jpg",
            verticalScroll_decButton_frameColor=(
                0,
                0,
                0,
                0),
            verticalScroll_decButton_scale=0.04,
            verticalScroll_decButton_image_scale=0.04)
    ...
    def scrollUp(self):
        if self.visible:
            self.serverList["verticalScroll_value"] = max(0, min(
                1, self.serverList["verticalScroll_value"] - self.serverList["verticalScroll_scrollSize"]))
    ...
    def showHosts(self, hosts):
        if not self.active:
            # In case the lobby calls this callback after the menu has been
            # deleted.
            return
        ...
        self.serverList = DirectScrolledFrame(
            ...
            verticalScroll_frameColor=(
                ...
                0.8,
                0.4),
            verticalScroll_pageSize=0.5,
            verticalScroll_scrollSize=0.5,
            verticalScroll_thumb_rolloverSound=None,
            verticalScroll_thumb_clickSound=None,
            verticalScroll_incButton_rolloverSound=None,
            verticalScroll_incButton_clickSound=None,
            ...
            verticalScroll_incButton_image="images/scroll-down.jpg",
            verticalScroll_incButton_frameColor=(
                0,
                0,
                0,
            ...
            verticalScroll_incButton_scale=0.04,
            verticalScroll_incButton_image_scale=0.04,
            verticalScroll_decButton_image="images/scroll-up.jpg",
            verticalScroll_decButton_frameColor=(
                0,
                0,
                0,
        ...
        for (user, map, host, players, playerSlots) in hosts:
            self.hostButtons.append(DirectButton(parent=self.serverList.getCanvas(), text=user + " - " + map + " (" + str(players) + "/" +
                                                 str(playerSlots) + ")", text_align=TextNode.ALeft, pos=(0, 0, offset), relief=DGG.FLAT, text_font=dejavuFont,
                                                 frameColor=(0.1, 0.4, 0.6, 0.6), frameSize=(-0.95, 0.95, -.075, .075), text_fg=(1, 1, 1, 1), text_scale=0.05,
                                                 text_pos=(-0.9, -0.02), scale=0.8, rolloverSound=None, clickSound=None, command=self.go, extraArgs=[host], suppressMouse=0))
...
class LoginDialog(DirectObject):
    ...
    def __init__(self, callback):
        ...
        visitorFont = loader.loadFont("menu/visitor2.ttf")
        self.dialog = DirectFrame(frameColor=(
            0.0, 0.0, 0.0, 0.0), frameSize=(-.7, .7, -.15, .15), pos=(0, 0, 0))
        self.background = OnscreenImage(
            image="menu/ui_background.png", pos=(
                0, 0, 0), parent=self.dialog, scale=(
                1, 1, .15 / .7), )
        self.background.setTransparency(TransparencyAttrib.MAlpha)
        self.background.setColor(1, 1, 1, 0.5)
        self.label = OnscreenText(
            parent=self.dialog, text="Enter a username", pos=(
                0, .07), font=visitorFont, fg=(
                1, 1, 1, 1), scale=0.1)
        self.usernameEntry = DirectEntry(
            ...
            frameColor=(
                ...
                0,
                0.5),
            initialText=engine.savedUsername,
            numLines=1,
            rolloverSound=None,
            clickSound=None,
            focus=1,
        ...
        self.loginButton = DirectButton(
            parent=self.dialog, text="Login", pos=(
                0.4, 0, -.03), relief=DGG.FLAT, text_font=visitorFont, frameColor=(
                0, 0, 0, 0.5), frameSize=(
                -0.4, 0.4, -.14, .14), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.475, rolloverSound=None, clickSound=None, command=self.go)
        self.dialog.setScale(0.0)
        self.dialog.hide()
        self.lastShow = -1
        self.lastHide = -1
...
class MapList(DirectObject):
    ...
    def __init__(self, callback):
        ...
        self.background.setTransparency(TransparencyAttrib.MAlpha)
        self.background.setColor(1, 1, 1, 0.5)
        self.label = OnscreenText(parent=self.dialog, text="Choose map", pos=(
            0, .825), font=visitorFont, fg=(1, 1, 1, 1), scale=0.1)
        self.cancelButton = DirectButton(
            parent=self.dialog, text="Cancel", pos=(
                0, 0, -0.8), relief=DGG.FLAT, text_font=visitorFont, frameColor=(
                0, 0, 0, 0.5), frameSize=(
                -0.5, 0.5, -.15, .15), text_fg=(
                    1, 1, 1, 1), text_scale=0.3, text_pos=(
                        0, -0.04), scale=0.3, rolloverSound=None, clickSound=None, command=self.hide)
        ...
        self.mapList = DirectScrolledFrame(
            parent=self.dialog,
            pos=(
                0,
                0,
        ...
        for map in engine.maps:
            ...
            self.mapButtons.append(DirectButton(parent=self.mapList.getCanvas(), text=title, text_align=TextNode.ALeft, pos=(0, 0, offset),
                                                relief=DGG.FLAT, text_font=visitorFont, frameColor=(0.1, 0.4, 0.6, 0.6), frameSize=(-0.95, 0.95, -.075, .075),
                                                text_fg=(1, 1, 1, 1), text_scale=0.1, text_pos=(-0.9, -0.02), scale=0.8, rolloverSound=None, clickSound=None,
                                                command=self.callback, extraArgs=[name, mapType], suppressMouse=0))
    ...
    def scrollUp(self):
        if self.visible:
            self.mapList["verticalScroll_value"] = max(0, min(
                1, self.mapList["verticalScroll_value"] - self.mapList["verticalScroll_scrollSize"]))
